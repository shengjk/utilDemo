...
class BFS private[graphframes] (private val graph: GraphFrame)
  ...
  def fromExpr(value: String): this.type = fromExpr(SQLHelpers.expr(value))
  ...
  def toExpr(value: String): this.type = toExpr(SQLHelpers.expr(value))
  ...
  def edgeFilter(value: String): this.type = edgeFilter(SQLHelpers.expr(value))
